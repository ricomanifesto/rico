...
const checks = [
  ...
  {
    label: "portfolio shell uses shared page background tokens",
    pattern: /--portfolio-page-background:\s*#0a192f;[\s\S]*--portfolio-page-foreground:\s*#e6f1ff;[\s\S]*--portfolio-header-background:\s*rgba\(10,\s*25,\s*47,\s*0\.95\);[\s\S]*body\s*\{[\s\S]*background-color:\s*var\(--portfolio-page-background\);[\s\S]*color:\s*var\(--portfolio-page-foreground\);/,
    source: indexCss,
  },
  {
  ...
  },
  {
    ...
    pattern: /\.portfolio-section\s*\{[\s\S]*scroll-margin-top:\s*9rem;[\s\S]*padding:\s*4rem 1rem;[\s\S]*background-color:\s*var\(--portfolio-page-background\);[\s\S]*color:\s*var\(--portfolio-page-foreground\);[\s\S]*@media \(min-width: 900px\)\s*\{[\s\S]*\.portfolio-section\s*\{[\s\S]*scroll-margin-top:\s*5rem;[\s\S]*<section id="about"[^>]*className="portfolio-section"[\s\S]*<section id="projects"[^>]*className="portfolio-section"[\s\S]*<section id="experience"[^>]*className="portfolio-section"/,
    source: `${indexCss}\n${aboutMeSource}\n${projectsSection}\n${experienceSource}`,
  },
  {
  ...
  },
  {
    ...
    pattern: /--portfolio-muted:\s*#8892b0;[\s\S]*\.footer-shell\s*\{[\s\S]*padding:\s*5rem 1rem 2\.5rem;[\s\S]*border-top:\s*1px solid var\(--portfolio-surface-border\);[\s\S]*background-color:\s*var\(--portfolio-page-background\);[\s\S]*color:\s*var\(--portfolio-muted\);[\s\S]*\.footer-content\s*\{[\s\S]*max-width:\s*56rem;[\s\S]*margin-right:\s*auto;[\s\S]*margin-left:\s*auto;[\s\S]*text-align:\s*center;[\s\S]*\.footer-copy\s*\{[\s\S]*font-size:\s*0\.875rem;[\s\S]*line-height:\s*1\.5rem;[\s\S]*className="footer-shell"[\s\S]*className="footer-content"[\s\S]*className="footer-copy"/,
    source: `${indexCss}\n${footerSource}`,
  },
  {
  ...
  },
  {
    ...
    pattern: /--portfolio-active-accent:\s*#66b2ff;[\s\S]*--portfolio-experience-tab-selected:\s*rgba\(17,\s*34,\s*64,\s*0\.78\);[\s\S]*--portfolio-experience-tab-hover:\s*rgba\(35,\s*53,\s*84,\s*0\.52\);[\s\S]*--portfolio-experience-tab-rail:\s*rgba\(136,\s*146,\s*176,\s*0\.22\);[\s\S]*\.experience-panel:focus-visible\s*\{[\s\S]*outline:\s*2px solid var\(--portfolio-focus-ring\);[\s\S]*\.experience-company-accent\s*\{[\s\S]*color:\s*var\(--primary-accent\);[\s\S]*\.experience-period\s*\{[\s\S]*color:\s*var\(--portfolio-muted\);[\s\S]*\.experience-highlight\s*\{[\s\S]*display:\s*flex;[\s\S]*\.experience-highlight-icon\s*\{[\s\S]*color:\s*var\(--primary-accent\);[\s\S]*\.experience-highlight-copy\s*\{[\s\S]*color:\s*var\(--portfolio-copy\);[\s\S]*\.experience-tab\s*\{[\s\S]*border-left:\s*1px solid var\(--portfolio-experience-tab-rail\);[\s\S]*border-radius:\s*0 var\(--radius\) var\(--radius\) 0;[\s\S]*transition:\s*background-color 300ms ease,\s*color 300ms ease,\s*border-color 300ms ease;[\s\S]*\.experience-tab:focus-visible\s*\{[\s\S]*outline:\s*2px solid var\(--portfolio-focus-ring\);[\s\S]*\.experience-tab-selected\s*\{[\s\S]*border-color:\s*var\(--primary-accent\);[\s\S]*background-color:\s*var\(--portfolio-experience-tab-selected\);[\s\S]*color:\s*var\(--portfolio-active-accent\);[\s\S]*\.experience-tab-idle\s*\{[\s\S]*color:\s*var\(--portfolio-muted\);[\s\S]*\.experience-tab-idle:hover\s*\{[\s\S]*background-color:\s*var\(--portfolio-experience-tab-hover\);[\s\S]*color:\s*var\(--portfolio-copy\);[\s\S]*\.experience-tab-indicator-selected\s*\{[\s\S]*background-color:\s*var\(--primary-accent\);[\s\S]*className="experience-panel"[\s\S]*<span className="experience-company-accent">[\s\S]*className="experience-period"[\s\S]*className="experience-highlight"[\s\S]*className="experience-highlight-icon"[\s\S]*className="experience-highlight-copy"[\s\S]*className=\{`experience-tab \$\{tabViewState\.textClass\}`\}[\s\S]*className=\{`experience-tab-indicator \$\{tabViewState\.indicatorClass\}`\}/,
    source: `${indexCss}\n${experienceSource}`,
  },
  {
  ...
  },
  {
    ...
    pattern: /\.experience-tab-selected\s*\{[\s\S]*border-color:\s*var\(--primary-accent\);[\s\S]*background-color:\s*var\(--portfolio-experience-tab-selected\);[\s\S]*color:\s*var\(--portfolio-active-accent\);[\s\S]*\.experience-tab-idle\s*\{[\s\S]*color:\s*var\(--portfolio-muted\);[\s\S]*\.experience-tab-idle:hover\s*\{[\s\S]*background-color:\s*var\(--portfolio-experience-tab-hover\);[\s\S]*color:\s*var\(--portfolio-copy\);[\s\S]*interface ExperienceTabViewState \{[\s\S]*readonly tabIndex:\s*0 \| -1;[\s\S]*readonly textClass:\s*string;[\s\S]*indicatorClass:\s*string;[\s\S]*getExperienceTabViewState[\s\S]*textClass:\s*isSelected \? "experience-tab-selected" : "experience-tab-idle"[\s\S]*function ExperienceTab\([\s\S]*const tabViewState = getExperienceTabViewState\(isSelected\);[\s\S]*className=\{`experience-tab \$\{tabViewState\.textClass\}`\}/,
    source: `${indexCss}\n${experienceSource}`,
  },
  {
  ...
  },
  {
    ...
    pattern: /href=\{siteBrand\.href\}[\s\S]*className="[^"]*header-brand-link[^"]*"[\s\S]*>\s*<img[\s\S]*src="\/favicon\.svg"[\s\S]*alt=""[\s\S]*aria-hidden="true"[\s\S]*className="header-brand-mark"[\s\S]*\/?>[\s\S]*<span>\{siteBrand\.label\}<\/span>\s*<\/a>/,
    source: headerSource,
  },
  {
